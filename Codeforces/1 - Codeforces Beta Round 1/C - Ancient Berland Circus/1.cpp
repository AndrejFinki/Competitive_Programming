...
int main()
{
    ...
    for( int i = 0 ; i < 3 ; i++ )
		for( int j = i + 1 ; j < 3 ; j++ )
			angles.push_back( Geometry::angle( points[i], out_circle.o, points[j] ) );

    for( int N = 3 ; N <= 101 ; N++ ) {
        bool valid = true;
        for( const LD &angle : angles ){
            LD k = angle * N / 360.0L;
            if( Geometry::are_equal( k, nearbyint( k ) ) ) continue;
			valid = false;
			break;
        }

        if( !valid ) continue;
        cout << out_circle.r * out_circle.r * N * sin( Geometry::deg_to_rad( 360.0L / N ) ) / 2 << endl;
		break;
    }
}