...
int main(){ ios::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);
    ...
    string dateString; cin >> dateString;

    Date currentDate(dateString);
    int A[3] = { currentDate.D, currentDate.M, currentDate.Y };
    sort( A, A+3 );
    
    Date earliestDate( 1, 1, 3000 );
    do{

        currentDate = { A[0], A[1], ( A[2] < 2000 ? 2000+A[2] : A[2] ) };
        if( currentDate.isValid() && currentDate < earliestDate ) earliestDate = currentDate;

    }while( next_permutation( A, A+3 ) );

    if( earliestDate.Y == 3000 ) cout << dateString << " is illegal" << endl;
    else                         cout << earliestDate.returnDate("|Y-|M-|D") << endl;
