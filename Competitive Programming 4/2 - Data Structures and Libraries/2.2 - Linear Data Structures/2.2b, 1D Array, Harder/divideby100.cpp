...
int main(){ ios::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);
    ...
    string ADD = "";
    while( N.length() + ADD.length() < M.length() ){
        ADD += "0";
    }
    N = ADD + N;

    int dot = N.length() - (M.length() - 1);

    int cap = INF;
    for( int i = N.length() - 1 ; i >= dot ; i-- ){
        if( N[i] == '0' ) cap = i;
        else break;
    }
    N = N.substr(0,cap);

    if( dot >= N.size() ) cout << N << endl;
    else cout << N.substr(0,dot) << "." << N.substr(dot,INF) << endl;
