...
int disorder( int[], int, int );

const int maxN = 100005;
int N;
int A1[maxN], A2[maxN], T[maxN];
...
int main(){ ios::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);
    ...
    int I1 = disorder( A1, 0, N-1 );
    int I2 = disorder( A2, 0, N-1 );

    if( bool(I1&1) == bool(I2&1) ) cout << "Possible" << endl;
    else cout << "Impossible" << endl;

    if( DEBUG ){
        PrintA(A1,N);
        PrintA(A2,N);
        cout << Print(I1) << Print(I2) << endl;
    }
...
int disorder( int A[], int L, int R ){

    if( L == R ) return 0;

    int mid = (L+R) / 2;
    int _L = disorder( A, L, mid );
    int _R = disorder( A, mid+1, R );
    int rv = 0;

    int _l = L, _r = mid+1, _i = L;
    while( _l <= mid && _r <= R ){
        if( A[_l] <= A[_r] ){ T[_i++] = A[_l++]; continue; }
        T[_i++] = A[_r++];
        rv += (mid-_l+1);
    }
    while( _l <= mid ) T[_i++] = A[_l++];
    while( _r <= R )   T[_i++] = A[_r++];
