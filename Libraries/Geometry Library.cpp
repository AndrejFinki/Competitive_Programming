...
struct circle{
    ...
    LD chord( LD ANG ){ return 2*r*sin(DEGtoRAD(ANG)/2); }
    ...
    bool contains( point T ){
        return r*r - (T.x-O.x)*(T.x-O.x) - (T.y-O.y)*(T.y-O.y) > -ERR; // including perimeter
        return r*r - (T.x-O.x)*(T.x-O.x) - (T.y-O.y)*(T.y-O.y) > 0; // only inside
    }
};
...
struct triangle{
    ...
    LD baseArea(){
        if( isValidNumber( ha.T1.x ) && isValidNumber( a.T1.x ) ) return 0.5 * ha_ * a_;
        if( isValidNumber( hb.T1.x ) && isValidNumber( b.T1.x ) ) return 0.5 * hb_ * b_;
        if( isValidNumber( hc.T1.x ) && isValidNumber( c.T1.x ) ) return 0.5 * hc_ * c_;
        return NAN;
    }
    LD perimeter(){
        return a_ + b_ + c_;
    ...
    bool contains( point P ){
        ...
        return 0-ERR <= L1 && L1 <= 1+ERR && 0-ERR <= L2 && L2 <= 1+ERR && 0-ERR <= L3 && L3 <= 1+ERR; // including perimeter
        return 0 < L1 && L1 < 1 && 0 < L2 && L2 < 1 && 0 < L3 && L3 < 1; // only inside
...
struct rectangle{
    ...
    rectangle( point pa, point pb, bool paIsUL ){
        rectangle::reset();
        if( paIsUL ){
            UL = pa; BR = pb;
            BL = point( BR.x, UL.y );
    ...
    bool contains( point P ){
        ...
        return minX < P.x && minY < P.y && maxX > P.x && maxY > P.y; // not including side
    }
    void print(){
        cout << "= = = = = = = = = = = = = = = =" << endl;
...
struct polygon{
    ...
    bool contains( point pt ){
        ...
        for( int i=0 ; i < N-1 ; i++ ){
            ...
            if( ccw(pt, P[i], P[i + 1]) ) SUM += DEGtoRAD(angle(P[i], pt, P[i + 1]));
            else                          SUM -= DEGtoRAD(angle(P[i], pt, P[i + 1]));
        }
        // cout << fixed << setprecision(10) << SUM << ' ' << fabs(fabs(SUM) - 2*PI) << endl;
...
int positionRelativeToCircle( point, circle );
...
bool canFormTriangle( lineSegment, lineSegment, lineSegment );
bool canFormTriangle( point, point, point );
bool canFormSquare( LD, LD, LD, LD );
bool canFormRectangle( LD, LD, LD, LD );
bool canFormQuadrilateral( LD, LD, LD, LD );
...
polygon convexHull( vector <point>& );
...
LD distanceNSQ( point A, point B ){
...
bool ccw( point A, point B, point C ){
    ...
    return CROSS > 0 || isZero( CROSS ); // isZero for collinear points acceptance
}
...
point intersectionPoint( line A, line B ){
    ...
    if( isNonvalidLine(A) || isNonvalidLine(B) ) return INVALID_POINT;
...
int positionRelativeToCircle( point P, circle C ){
    point _P( P.x - C.O.x, P.y - C.O.y );
    LD _E = _P.x*_P.x + _P.y*_P.y, _R = C.r*C.r;
    if( areEqual( _E, _R ) ) return 0; // BORDER
    if( _E < _R ) return -1; // INSIDE
    return 1; // OUTSIDE
}
...
int intersectionPoints( line A, circle C, point &IP1, point &IP2 ){
    ...
    if( isZero( A.b ) ){
        ...
        LD DELTA = C.r*C.r - A.c*A.c/(A.a*A.a);
        ...
        IP1 = point( (-_B + sqrt( DELTA )) / (2*_A), A.a*((-_B + sqrt( DELTA )) / (2*_A)) + A.c );
        IP2 = point( (-_B - sqrt( DELTA )) / (2*_A), A.a*((-_B - sqrt( DELTA )) / (2*_A)) + A.c );
...
bool canFormTriangle( lineSegment a, lineSegment b, lineSegment c ){
    return a.length() + b.length() > c.length() && a.length() + c.length() > b.length() && b.length() + c.length() > a.length();
}
bool canFormTriangle( point A, point B, point C ){
    return canFormTriangle( lineSegment(A,B), lineSegment(B,C), lineSegment(A,C) );
}

// QUADRILATERAL FUNCTIONS //

bool canFormSquare( LD a, LD b, LD c, LD d ){
    return areEqual( a, b ) && areEqual( b, c ) && areEqual( c, d );
}
bool canFormRectangle( LD a, LD b, LD c, LD d ){
    return ( areEqual( a, b ) && areEqual( c, d ) ) || ( areEqual( a, c ) && areEqual( b, d ) ) || ( areEqual( a, d ) && areEqual( b, c ) );
}
bool canFormQuadrilateral( LD a, LD b, LD c, LD d ){
...
rectangle intersectionRectangle( rectangle A, rectangle B ){
    ...
    for(int i=0;i<L1.size();i++) for(int j=0;j<L2.size();j++){
        ...
        if( isValidNumber( IP.x ) ) intersectionPoints.push_back( IP );
    }
...
polygon intersectionPolygon( polygon A, polygon B ){
    ...
    for(int i=0;i<A.N-1;i++){
        for(int j=0;j<B.N-1;j++){
            ...
            // cout << A.P[i].x << ',' << A.P[i].y << " -> " << A.P[i+1].x << ',' << A.P[i+1].y << endl;
            // cout << B.P[j].x << ',' << B.P[j].y << " -> " << B.P[j+1].x << ',' << B.P[j+1].y << endl;
            // IP.print();
            if( isValidNumber( IP.x ) ) pointSet.push_back( IP );
        }
    }

    return convexHull( pointSet );
...
int main(){ ios::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);
    ...
    point O = T.outCirclePoint();
    LD R = T.outCircleRadius();

    vector <LD> ANG;

    for(int i=0;i<3;i++) for(int j=i+1;j<3;j++) ANG.push_back( angle( P[i], O, P[j] ) );
    if( DEBUG ) for(int i=0;i<3;i++) for(int j=i+1;j<3;j++) cout << i << ' ' << j << " = " << angle( P[i], O, P[j] ) << endl;
    ...
    for( int N=3;N<=101;N++ ){
        ...
        for( LD a : ANG ){
            ...
            LD k = a * N / 360.0L;
            if( DEBUG ) cout << k << ' ' << nearbyint(k) << endl;
            if( !areEqual( k, nearbyint(k) ) ){ valid = false; break; }

        }

        if( DEBUG ) cout << N << ' ' << valid << ' ' << R*R*N*sin( DEGtoRAD(360.0L/N) )/2 << endl;

        if( !valid ) continue;
        
        cout << R*R*N*sin( DEGtoRAD(360.0L/N) )/2 << endl; break;
